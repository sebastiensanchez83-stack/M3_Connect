...
//  - staff kinds (confirmed/paid/ecat_published/info_requested/declined/payment_reminder/invoice_available): admin/moderator only
...
function content(kind: string, r: Reg, pay?: Pay | null): { subject: string; html: string; to: string } | null {
  ...
  switch (kind) {
    ...
    case "invoice_available": {
      ...
      const amount = pay?.amount_cents != null ? `${(pay.amount_cents / 100).toFixed(2)} ${cur}` : null;
      const detail = amount
        ? `<p>Amount due: <strong>${amount}</strong> (VAT excl.)${pay?.invoice_ref ? `<br>Invoice reference: <strong>${pay.invoice_ref}</strong>` : ""}</p>`
        : "";
      return { to, subject: `Your invoice is available — ${EVENT}`,
        html: `${hi}<p>Your invoice for the <strong>${EVENT}</strong> (${WHEN}) is now available in your event hub. You can view and download it from your account at any time.</p>${detail}<p>For any question about your invoice, contact <a href="mailto:${ADMIN_EMAIL}">${ADMIN_EMAIL}</a>.</p>${btn(`${SITE_URL}/account?tab=event`, "View my invoice")}${foot}` };
    }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // For payment_reminder / invoice_available, read the saved payment so the figures are authoritative.
    let pay: Pay | null = null;
    if (kind === "payment_reminder" || kind === "invoice_available") {
      ...
      // The invoice email quotes the figure of the invoice document itself (the
      // newest sm_invoice row), falling back to the payment record — sm_payment
      // may hold an older, different amount than the invoice just uploaded.
      if (kind === "invoice_available") {
        const { data: inv } = await admin.from("sm_invoice").select("amount_cents, currency").eq("registration_id", registration_id).order("created_at", { ascending: false }).limit(1).maybeSingle();
        const i = inv as { amount_cents?: number | null; currency?: string | null } | null;
        if (i?.amount_cents != null) pay = { ...(pay || {}), amount_cents: i.amount_cents, currency: i.currency || pay?.currency };
      }
    }
